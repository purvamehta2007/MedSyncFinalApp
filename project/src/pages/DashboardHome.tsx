...
import { supabase, Medicine, Reminder, Intake, HealthReading } from '../lib/supabase';
import { useAuth } from '../lib/auth-context';
import { Package, Bell, CircleCheck as CheckCircle, CircleAlert as AlertCircle, TrendingUp, Calendar, Activity } from 'lucide-react';
...
export function DashboardHome() {
  ...
  const loadDashboardData = async () => {
    try {
      const [medicinesRes, remindersRes, intakesRes, readingsRes] = await Promise.all([
      ...
      if (readingsRes.data) setHealthReadings(readingsRes.data);
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">Health Readings Trend</h2>
          <Link to="/dashboard/health-readings" className="text-blue-500 hover:text-blue-600 text-sm font-medium">
            View all
          </Link>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">BP (Sys/Dia)</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Blood Sugar</th>
                ...
                {healthReadings.map((reading) => (
                  <tr key={reading.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                      {format(new Date(reading.reading_date), 'MMM dd')}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      {reading.blood_sugar ? `${reading.blood_sugar} mg/dL` : '-'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">
